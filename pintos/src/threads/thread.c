...
/* 	init_thread and thread_yield and thread_unblock -> use list_insert_ordered to replace list_push_back		*/
}
...
void check_blocked_threads( struct thread *t, void *aux UNUSED){
...
void
thread_mlfqs_recent_cpu_plusplus (void)
{
  ASSERT (thread_mlfqs);
  ASSERT (intr_context ());

  struct thread *current_thread = thread_current ();
  if (current_thread == THREAD_READY)
    return;
